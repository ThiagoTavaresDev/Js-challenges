...
*/
const my3FavoriteTVShows = ['The Good Place', 'You', 'La Casa De Papel']
const sentence = `Minha 3 séries favoritas são: ${my3FavoriteTVShows.slice(0,2)} e ${my3FavoriteTVShows.slice(2,3)}`
...
let isRandomTVShowIncluded = false
if (my3FavoriteTVShows.includes(randomTVShow)){
    const isRandomTVShowIncluded = true
    console.log(isRandomTVShowIncluded)
    
} 
    else 
    { 
     const isRandomTVShowIncluded = false
     console.log(isRandomTVShowIncluded)
...
/*
  ...
  caracteres? BOOLEAN."
*/
console.log(`"A string que a "typeSentence" armazena tem mais de 39 caracteres? ${typeSentence.length == 39 ? true : false }"`)
...
// null é tratado como zero em operações aritméticas
...
*/
const isNotAString = randomTVShow ==! randomTVShow.toString() ? true : false
...
console.log(8 == evenNumbers.find(number => number == 8) ? true : false)