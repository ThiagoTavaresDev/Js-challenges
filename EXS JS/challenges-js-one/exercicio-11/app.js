...
invokeName()
// por conta do escopo de função o let não foi transpassado para fora, sendo assim podemos definir outro let com o mesmo nome da variável que esta dentro da função console.log(idade)
...
const car = {
    ...
    isRunning: false,
    run: () =>{
        console.log(`${car.name} está em movimento`)
    },
    stop: () =>{
        console.log(`${car.name} está parado`)
    },
    getColorsMessage: ()=>{
        console.log(`${car.name} está disponível nas cores ${car.colors}`)
...
console.log(`O carro é uma marca ${car['brand']} nome ${car['name']}`);
