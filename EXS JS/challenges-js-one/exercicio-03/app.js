...
*/
const foodsInfo =  `até aqui, o array "braziliandFoods" possui ${brazilianFoods.length} itens: ${brazilianFoods[0]} ${brazilianFoods[1]} ${brazilianFoods[2]}`
...
agora é "Lilica".
*/
const lastItem = dogNames.length - 1

console.log(dogNames[lastItem])
...
const initials = `${dogNames.map(name => name[0]).join('')}`;
...
dogNames.splice(1, 1)
const dessert = `${dogNames.map(letters => letters.slice(-2))}`
console.log(dessert.replace(/[,]/g,'').replace('n','d'))
