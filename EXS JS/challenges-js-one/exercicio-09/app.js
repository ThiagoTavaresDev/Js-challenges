...
console.log(convertToString(['fala', 20, 10, 'opa']))
...
const quantifyCharacter = (string) =>{
    return string.length

}
console.log(quantifyCharacter('opa'))
/*
  03
...
const indexCharacter = (character, string) =>{
    return string.indexOf(character)

}
console.log(indexCharacter('a', 'opa'))
...
*/
const verifyExist = (item, array) => {
    return array.includes(item)

}
console.log(verifyExist('abacaxi', ['abacaxi', 'uva', 'maça']))
...
console.log(concatArray(['opa', 2], ['fala',3]))
...
console.log(popArray(['fala','tudo bem', 30, 50]))
...
const callFunction = (anotherFunction) =>{
...
console.log(callFunction(anotherFunction('jose')))
...
const invokeFunction = (callback) => {
    return callback
}
function callback(number){
    return number * 3
}
console.log(invokeFunction(callback(33)))
...
numbers.forEach(number => console.log(`o ${number} item do array ${numbers} é ${number}`))
...
// funcao cria e insere
function creatAndInsert(){
    // aqui nós criamos um foreach que acessa cada item do array review, junto deste loop aproveitamos para criar os paragráfos e já inserirmos os respectivos textos, pendurando também os paragrafos com os textos na section
    review.forEach((text) => { 
        let paragrafs = document.createElement('p')
        paragrafs.textContent = text
        section.appendChild(paragrafs)
    }) 
}
creatAndInsert()
...
*/
const arrayEx = ['joao', 'marcia', 'Carlos', 'jose','karina','pedrin']
const verifyLike = (array) =>{
    if(array.length === 0){
        return console.log('ninguém curtiu isso')
    }
    else if(array.length === 1){
        return console.log(`${array[0]} curtiu isso`)
    }
    else if(array.length === 2){
        return console.log(`${array[0]} e ${array[1]} curtiram isso`)
    }
    else if(array.length === 3){
        return console.log(`${array[0]}, ${array[1]} e ${array[2]} curtiram isso`)
    }
    else if(array.length > 4){
        return console.log(`${array[0]}, ${array[1]} e mais ${array.length - 2} curtiram isso`)
    }
}
console.log(verifyLike(arrayEx))